...
<div class="content-wrapper">
  ...
  <section class="invoice" style="margin: 15px;">
      ...
      <div class="row">
        ...
        <div class="col-xs-12">
          <h2 class="page-header" style="padding-bottom: 25px; margin-top:0;">
            <i class="fa fa-globe"></i> {{ trans('labels.OrderID') }}# {{ $data['orders_data'][0]->order_id }}
      ...
      <div class="row invoice-info">
        ...
        <div class="col-sm-4 invoice-col">
          ...
          <address>
           ...
           <strong>Landmark:</strong> {{ $data['orders_data'][0]->delivery_suburb }} <br>
            <strong>Address:</strong> {{ $data['orders_data'][0]->delivery_street_address }} <br>
          ...
          <table class="table table-striped">
            ...
            <tr>
                ...
                @foreach($products->attribute as $attributes)
                	<b>{{ trans('labels.Name') }}:</b> {{ $attributes->products_options_name }}<br>
                    <b>{{ trans('labels.Value') }}:</b> {{ $attributes->products_options_values_name }}<br>
      ...
      <div class="row">
        ...
        <div class="col-xs-7">
          ...
          <p class="text-muted well well-sm no-shadow" style="text-transform:capitalize">
              {{ str_replace('_',' ', $data['orders_data'][0]->payment_method) }},&nbsp;
                <span>Status : {{ str_replace('_',' ', $data['orders_data'][0]->payment_status) }}</span>
          </p>
          ...
          <div class="table-responsive ">
            <table class="table order-table">
              ...
              <tr>
                ...
                <td>
                  @if(!empty($result['commonContent']['currency']->symbol_left)) {{$result['commonContent']['currency']->symbol_left}} @endif {{ $data['orders_data'][0]->net_amount }} @if(!empty($result['commonContent']['currency']->symbol_right)) {{$result['commonContent']['currency']->symbol_right}} @endif</td>
                  </td>
              </tr>
            </table>
          </div>
        ...
        @if($data['orders_data'][0]->orders_status_id != '2' || $data['orders_data'][0]->orders_status_id != '3' || $data['orders_data'][0]->orders_status_id != '6')
          ...
          <p class="lead">{{ trans('labels.ChangeStatus') }}: </p>
            ...
            <div class="col-md-12">
              <div class="form-group">
                ...
                <select class="form-control select2" id="status_id" required='' name="orders_status" style="width: 100%;">
                    <option value="">Select Status</option>
               	 @foreach( $data['orders_status'] as $orders_status)
                 @if($data['orders_data'][0]->orders_status_id == '5')
                     @if($orders_status->orders_status_id == '7')
                          ...
                          @endif 
                     @if($orders_status->orders_status_id == '3')
                          <option value="{{ $orders_status->orders_status_id }}" @if( $data['orders_data'][0]->orders_status_id == $orders_status->orders_status_id) selected="selected" @endif >{{ $orders_status->orders_status_name }}</option>
                          @endif 
                  @endif
                  @if($data['orders_data'][0]->orders_status_id == '7')
                    @if($orders_status->orders_status_id == '8')
                          <option value="{{ $orders_status->orders_status_id }}" @if( $data['orders_data'][0]->orders_status_id == $orders_status->orders_status_id) selected="selected" @endif >{{ $orders_status->orders_status_name }}</option>
                          @endif 
                     @if($orders_status->orders_status_id == '2')
                          <option value="{{ $orders_status->orders_status_id }}" @if( $data['orders_data'][0]->orders_status_id == $orders_status->orders_status_id) selected="selected" @endif >{{ $orders_status->orders_status_name }}</option>
                          @endif 
                  @endif
                  @endforeach
