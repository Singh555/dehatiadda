...
if(!function_exists('returnResponse')){
   function returnResponse($message='Started', $code = HttpStatus::HTTP_BAD_REQUEST, $status = HttpStatus::HTTP_ERROR, $data=null, $pager = null,$total_records=null){
       $api_call_id = session()->get('api_call_id');
       session()->forget('api_call_id');
       Log::debug(__CLASS__." :: ".__FUNCTION__." started with parameters as code $code, status $status, message $message api call list id $api_call_id");
       if(!empty($api_call_id)){
       DB::table('http_call_record')->where('id',$api_call_id)->update(['end_time'=>Carbon::now(),'response_time'=>microtime(true) - LARAVEL_START]);
       }
       $resp = new \stdClass();
       $resp->code = $code;
       $resp->status = $status;
...
if(!function_exists('uploadImage')){
    function uploadImage($file, $path, $type)
    {
        try 
        {
            $fileInFolder = public_path()."/images/".$path;
            $fileName = time().'-'.rand(1,9999).'-'.$type.".".$file->getClientOriginalExtension();
            $filePathDB = 'images/'.$path."/".$fileName;
...
if(!function_exists('getStatusLabel')){
    ...
    {
        $resp = '<span class="label label-default"><i class="fa fa-hand-point-right">&nbsp;</i>'.$status.'</span>';
        switch ($status) {
            case 'PENDING':
                $resp= '<span class="label label-warning"><i class="fas fa-hourglass-half">&nbsp;</i>'.$status.'</span>';
            ...
            case 'NO':
                ...
                break;
           case 'N':
                $resp= '<span class="label label-danger">'.$status.'</span>';

                break;
            case 'PAID':
                $resp= '<span class="label label-success"><i class="fa fa-check">&nbsp;</i>'.$status.'</span>';

                break;
            case 'CREDIT':
                $resp= '<span class="label label-success"><i class="fas fa-plus">&nbsp;</i>'.$status.'</span>';
            ...
            case 'APPROVED':
                ...
                break;
            case 'INACTIVE':
                $resp= '<span class="label label-danger"><i class="fas fa-ban">&nbsp;</i>'.$status.'</span>';
            ...
            case 'REJECTED':
                ...
                break;
            case 'DEBIT':
                $resp= '<span class="label label-danger"><i class="fas fa-minus">&nbsp;</i>'.$status.'</span>';
            ...
            case 'YES':
                ...
                break;
            case 'Y':
                $resp= '<span class="label label-success">'.$status.'</span>';

                break;
            case 'PROCESS':
                $resp= '<span class="label label-warning"><i class="fa fa-hourglass-half">&nbsp;</i>'.$status.'</span>';
...
if(!function_exists('callUrlWithHeader')){
    function callUrlWithHeader($url,$header,$postFields) {
