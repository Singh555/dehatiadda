...
class Setting extends Model
{
    ...
    {
        $setting = DB::table('video_links')->orderby('id', 'ASC')->paginate(20);
        return $setting;
    }
    ...
    public function commonContent()
    {
        ...
        $orders = DB::table('orders')
            ...
            foreach($orders as $orders_data){
              ...
              array_push($unseenOrders,$orders_data);
              $orders_products = DB::table('orders_products')
                ->where('orders_id', '=' ,$orders_data->orders_id)
                ->get();

              $unseenOrders[$index]->price = $orders_products;
              $unseenOrders[$index]->total_products = count($orders_products);
              $index++;
            }
            //new customers
