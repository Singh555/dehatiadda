    ...
    public static function updatebanner($request){
        ...
        if($request->image_id){
            if($request->hasFile('image_id')){
                $uploadImage = uploadImage($request->file('image_id'), 'constant_banners',$request->type);
                $oldImage = $request->oldImage;
            }else{
                $uploadImage = $request->oldImage;
            }
           $result = DB::table('constant_banners')->where('banners_id', $request->id)->update([
                'banners_title'  		 =>   $request->type,
                'date_added'	 		 =>   date('Y-m-d H:i:s'),
                'banners_image_url'			 =>	  $uploadImage,
                'banners_url'	 		 =>   $request->banners_url,
                'status'	 			 =>   $request->status,
                'type'					 =>	  $request->type,
                'languages_id'			 =>	  $request->languages_id
         ...
         return DB::table('constant_banners')->where('banners_id', $request->id)->update([
                'banners_title'  		 =>   $request->type,
                'date_added'	 		 =>   date('Y-m-d H:i:s'),
                'banners_url'	 		 =>   $request->banners_url,
                'status'	 			 =>   $request->status,
