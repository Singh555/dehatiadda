...
class HomeBanners extends Model
{
    ...
    foreach($languages as $language){
        ...
        $banners = HomeBanners::LeftJoin('image_categories', function ($join) {
            ...
            ->select('home_banners.home_banners_id', 'home_banners.image','home_banners.image_url', 'home_banners.created_at',
            ...
            if(count($banners)>0){
                foreach( $banners as $banner){   
                    ...
                    $description_data[$language->languages_id][$index]['path'] = $banner->image_url;
    ...
    public function updaterecord($banner_name, $language_id, $banners_text, $banners_image_id){
      if($banners_image_id!=''){
       return DB::table('home_banners')->where('banner_name', $banner_name)->where('language_id', $language_id)->update(
        [
            'text'   =>   addslashes($banners_text),
            'image_url'          =>   $banners_image_id,
            'created_at'     => date('y-m-d h:i:s'),
