...
class Categories extends Model
{
    ...
    public function paginator(){
      ...
      $categories = Categories::sortable(['categories_id'=>'ASC'])
           ...
           ->LeftJoin('image_categories as categoryTable', function ($join) {
            ...
            ->select('categories.categories_id as id', 'categories.categories_image as image','categories.categories_image_url','categories.categories_icon_url',
            'categories.app_home as app_home','categories.categories_icon as icon',  'categories.created_at as date_added',
    ...
    public function getter($language_id){
      $listingCategories = DB::table('categories')
          ...
          , 'categories.parent_id','categories.categories_image_url','categories.categories_icon_url')
    ...
    public function allcategories($language_id){
        $listingCategories = DB::table('categories')
            ->leftJoin('categories_description','categories_description.categories_id', '=', 'categories.categories_id')
            ->select('categories.categories_id as id', 'categories.categories_image as image',  'categories.created_at as date_added', 'categories.updated_at as last_modified', 'categories_description.categories_name as name', 'categories.categories_slug as slug','categories.categories_image_url','categories.categories_icon_url')
            ->where('categories_description.language_id','=', $language_id )
            ->where('categories_status', '1')
            ->get();
          ...
          case 'Name':
              $categories = Categories::sortable(['categories_id'=>'ASC'])
                    ->leftJoin('categories_description','categories_description.categories_id', '=', 'categories.categories_id')
                        ...
                        'categories.updated_at as last_modified', 'categories_description.categories_name as name',
                        'categories_description.language_id','categoryTable.path as imgpath','iconTable.path as iconpath','categories.categories_status  as categories_status','categories.categories_image_url','categories.categories_icon_url')
                        ->where('categories_description.language_id', '1')
                        ->where(function($query) {
                            $query->where('categoryTable.image_type', '=',  'THUMBNAIL')
                                ->where('categoryTable.image_type','!=',   'THUMBNAIL')
          ...
          case 'Main':
              ...
              $categories = Categories::sortable(['categories_id'=>'ASC'])
                  ...
                  ->select(
                      ...
                      'categories_description.language_id','categoryTable.path as imgpath','iconTable.path as iconpath'
                          ,'categories.categories_image_url','categories.categories_icon_url'
                  )
    ...
    public function insert($uploadImage,$date_added,$parent_id,$uploadIcon,$categories_status,$categories_app_home){
        $categories = DB::table('categories')->insertGetId([
            'categories_image_url'   =>   $uploadImage,
            'created_at'		 =>   $date_added,
            'parent_id'		 	 =>   $parent_id,
            'categories_icon_url'	 =>	  $uploadIcon,
            'categories_slug'    =>   'Null',
            'app_home'=>$categories_app_home,
    ...
    public function edit($request){
        $category = DB::table('categories') ->leftJoin('images','images.id', '=', 'categories.categories_image')
            ...
            'ImageTable.path as imagepath','IconTable.path as iconpath','categories.categories_image_url','categories.categories_icon_url')
            ->where('categories.categories_id', $request->id)->get();
        return $category;
    }

    public function updaterecord($categories_id,$uploadImage,$uploadIcon,$last_modified,$parent_id,$slug,$categories_status,$categories_app_home){
    ...
    public function editsubcategory($request){
        $editSubCategory = DB::table('categories')
            ...
            'categories.categories_slug as slug', 'categories.categories_status  as categories_status','categories.app_home  as app_home', 'categories.parent_id as parent_id','categoryTable.path as imgpath','iconTable.path as iconpath','categories.categories_image_url','categories.categories_icon_url')
            ->where('categories.categories_id', $request->id)->get();
    ...
    public function deleterecord($request){
        ...
        $image = $category->categories_image_url;
        $icon = $category->categories_icon_url;
        ...
        $unlink = public_path().'/'.$image;
                    $unlink2 = public_path().'/'.$icon;
                    if(!empty($image) && file_exists($unlink)){
                        unlink($unlink);
                    }
                    if(!empty($icon) && file_exists($unlink2)){
                        unlink($unlink2);
                    }
