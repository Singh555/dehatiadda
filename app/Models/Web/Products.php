...
class Products extends Model
{
    ...
    public function categories()
    {
        $items = DB::table('categories')
            ...
            ->select('categories.categories_id', 'categories.categories_image_url as image_path', 'categories.categories_slug as slug', 'categories_description.categories_name', 'categories.parent_id')
...
///////////////////////////////////////////////////////////////////////////////////////////////
    ...
    private function recursivecategories1()
    {
        $items = DB::table('categories')
            ->leftJoin('categories_description', 'categories_description.categories_id', '=', 'categories.categories_id')
            ->select('categories.categories_icon_url as icon', 'categories.categories_id', 'categories_description.categories_name', 'categories.categories_slug', 'categories.parent_id')
            ->where('categories_description.language_id', '=', Session::get('language_id'))
            ->where('categories.categories_status', 1)
            ->groupBy('categories.categories_id')
            ->get();
    ...
    public function products($data)
    {
        ...
        if ($type == "wishlist") {
            $categories->LeftJoin('liked_products', 'liked_products.liked_products_id', '=', 'products.products_id')
                ->select('products.*', 'products_image_url as image_path', 'products_description.*', 'manufacturers.*', 'manufacturers_info.manufacturers_url');

        }
        //parameter special
        elseif ($type == "special") {
            $categories->LeftJoin('specials', 'specials.products_id', '=', 'products.products_id')
                ->select('products.*', 'products_image_url as image_path', 'products_description.*', 'manufacturers.*', 'manufacturers_info.manufacturers_url', 'specials.specials_new_products_price as discount_price', 'specials.specials_new_products_price as discount_price');
        } elseif ($type == "flashsale") {
            //flash sale
            $categories->LeftJoin('flash_sale', 'flash_sale.products_id', '=', 'products.products_id')
                ->select(DB::raw(time() . ' as server_time'), 'products.*', 'products_image_url as image_path', 'products_description.*', 'manufacturers.*', 'manufacturers_info.manufacturers_url', 'flash_sale.flash_start_date', 'flash_sale.flash_expires_date', 'flash_sale.flash_sale_products_price as flash_price');

        } elseif ($type == "compare") {
            //flash sale
            $categories->LeftJoin('flash_sale', 'flash_sale.products_id', '=', 'products.products_id')
                ->select(DB::raw(time() . ' as server_time'), 'products.*', 'products_image_url as image_path', 'products_description.*', 'manufacturers.*', 'manufacturers_info.manufacturers_url', 'flash_sale.flash_start_date', 'flash_sale.flash_expires_date', 'flash_sale.flash_sale_products_price as discount_price');
            ...
            })->select('products.*', 'products_image_url as image_path', 'products_description.*', 'manufacturers.*', 'manufacturers_info.manufacturers_url', 'specials.specials_new_products_price as discount_price');
        }
        ...
        if (count($products) > 0) {
            ...
            foreach ($products as $products_data) {
                ...
                $products_id = $products_data->products_id;
                 ...
                 $products_data->default_images = $products_data->image_path;
                ...
                $default_image_thumb = DB::table('products')
                    ->select('products_image_url as image_path')
                    ->where('products_id', '=', $products_id)
                    ->first();
    ...
    public static function getquantity($request)
    {
        ...
        Log::debug(__CLASS__."::".__FUNCTION__." Called with product Id $products_id");
        //check products type
        ...
        if ($productsType[0]->products_type == 1) {
            ...
            $attributeid = implode(',', $attributes);
            Log::debug(__CLASS__."::".__FUNCTION__." Called with attributes Id $attributeid");
            $postAttributes = count($attributes);
