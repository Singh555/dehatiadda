...
class CategoriesController extends Controller
{
  ...
  public function display(){
    $title = Lang::get("labels.SubCategories");
    $categories = $this->Categories->paginator();
    $result['commonContent'] = $this->Setting->commonContent();
    return view("admin.categories.index",["title"=>$title])->with('categories', $categories)->with('result', $result);
  }
  ...
  public function filter(Request $request){
    ...
    $result['commonContent'] = $this->Setting->commonContent();
    return view("admin.categories.index", ["title"=>$title])->with('result', $result)->with(['categories'=> $categories, 'name'=> $request->FilterBy, 'param'=> $request->parameter]);
  }

  public function add(Request $request){
    $title = Lang::get("labels.AddSubCategories");
    ...
    $result['commonContent'] = $this->Setting->commonContent();
    return view("admin.categories.add",["title"=>$title])->with('result', $result)->with('allimage', $allimage);
  }
  ...
  public function insert(Request $request){
        ...
        $message  = '';
        try{
        $categories_app_home = $request->app_home;
        $categories_status  = $request->categories_status;
        if($request->hasFile('image_id')){
                $uploadImage = uploadImage($request->file('image_id'), 'category','Category');
            }
        if($request->hasFile('image_icone')){
                $uploadIcon = uploadImage($request->file('image_icone'), 'category','Icon');
            }

        $categories_id = $this->Categories->insert($uploadImage,$date_added,$parent_id,$uploadIcon,$categories_status,$categories_app_home);
        $slug_flag = false;
      ...
      Log::error(__CLASS__."::".__FUNCTION__."Exception occured :: ".$e->getMessage());
                    ...
                    $unlink2 = public_path().'/'.$uploadIcon;
                    if(!empty($uploadImage) && file_exists($unlink)){
                        unlink($unlink);
                    }
                    if(!empty($uploadIcon) && file_exists($unlink2)){
                        unlink($unlink2);
                    }
  }
  ...
  public function edit(Request $request){
    $title = Lang::get("labels.EditCategories");
    $images = new Images;
    $allimage = $images->getimages();
  ...
  //  dd($editSubCategory[0]->parent_id);
    ...
    $result['categories'] = $option;
    $result['commonContent'] = $this->Setting->commonContent();
    return view("admin.categories.edit",["title"=>$title])->with('result', $result)->with('allimage', $allimage);
   }

   public function update(Request $request){

     $title = Lang::get("labels.EditSubCategories");
     $result = array();
     ...
     $oldImage = '';$oldIcon = '';$uploadImage='';$uploadIcon='';
     try{
     //check slug
     ...
     $updateCategory = $this->Categories->updaterecord($categories_id,$uploadImage,$uploadIcon,$last_modified,$parent_id,$slug,$categories_status,$categories_app_home);
     ...
     foreach($languages as $languages_data){
       ...
       $checkExist = $this->Categories->checkExit($categories_id,$languages_data);
        ...
        $unlink2 = public_path().'/'.$oldIcon;
        if(!empty($oldImage) && file_exists($unlink)){
            unlink($unlink);
        }
        if(!empty($oldIcon) && file_exists($unlink2)){
            unlink($unlink2);
        }
