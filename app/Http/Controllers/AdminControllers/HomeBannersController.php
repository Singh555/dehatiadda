...
class HomeBannersController extends Controller {
    ...
    public function display() {
        $title = Lang::get("labels.Home Banners");
        $result['banners'] = $this->HomeBanners->index();
        $result['languages'] = $this->varseting->getLanguages();
        $allimage = $this->images->getimages();
        //dd($result);
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.settings.web.homebanners.index", ["title" => $title])->with('result', $result)->with('allimage', $allimage);
    }
            ...
            foreach ($banners_types as $key => $banners_type) {
                ...
                if ($checkExist) {
                    if ($request->hasFile($banner_image_id)) {
                        $banners_image_id = uploadImage($request->file($banner_image_id), 'home_banners', $banner_name);
                        $old_image_id = $checkExist->image_url;
                    } else {
                        $banners_image_id = $checkExist->image_url;
                    }
                    $result = $this->HomeBanners->updaterecord($banner_name, $languages_data->languages_id, $banners_text, $banners_image_id);

                    if ($result) {
                        $unlink = public_path() . '/' . $old_image_id;
                        if (!empty($old_image_id) && file_exists($unlink)) {
                            unlink($unlink);
                        }
                    } else {
                        $unlink = public_path() . '/' . $banners_image_id;
                        if (!empty($banners_image_id) && file_exists($unlink)) {
                            unlink($unlink);
                        }
                    }
                } else {
                   ...
                   $result = $this->HomeBanners->insertrecord($banner_name, $languages_data->languages_id, $banners_text, $banners_image_id);
                   if(!$result){
                       $unlink = public_path() . '/' . $banners_image_id;
                        if (!empty($banners_image_id) && file_exists($unlink)) {
                            unlink($unlink);
