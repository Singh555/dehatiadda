...
class AdminController extends Controller
{
	...
	public function dashboard(Request $request){
		$title 			  = 	Lang::get("labels.title_dashboard");
		$language_id      = 	'1';
		...
		foreach($orders as $orders_data){
			...
			if($orders_status_history->orders_status_id !== 3 || $orders_status_history->orders_status_id !== 4){
				foreach($orders_products as $orders_product){
					...
					if($single_stock>0){
                       ...
                       // Log::debug(__CLASS__."::".__FUNCTION__."Getting orders data as".json_encode($orders_data));
                        if($orders_data->orders_status =='COMPLETED'){
                            $sold_profit += $orders_data->net_amount;
                        }
  		...
  		$result['profit'] = number_format($sold_profit,2);
  		$result['purchase'] = number_format($sold_cost,2);
		...
		$recentProducts = DB::table('products')
			...
			->select('products.*', 'products_description.*', 'products.products_image_url as products_image')
		...
		return view("admin.dashboard",["title"=>$title])->with('result', $result);
	}
        ...
        public function media()
    {
       ...
       $result['commonContent'] = $this->Setting->commonContent();
            $result['commonContent'] = $this->Setting->commonContent();
            return view('admin.medias.add',["title"=>__('Media')])
            ->with('result', $result)
	...
	public function login(){
		...
		if (Auth::check()) {
		  return redirect('/admin/dashboard/this_month');
			...
			$title = Lang::get("labels.login_page_name");
                        $result['commonContent'] = $this->Setting->commonContent();
  ...
  public function checkLogin(Request $request){
		...
		if($validator->fails()){
			...
			$adminInfo = array("email" => $request->email, "password" => $request->password,"status"=>1);
                        Log::debug(__CLASS__."::".__FUNCTION__."Called with email ".$request->email."and password".$request->password);

			if(auth()->attempt($adminInfo)) {
				$admin = auth()->user();

				$administrators = DB::table('admins')->where('id', $admin->myid)->get();
	...
	public function adminProfile(Request $request){
		...
		return view("admin.profile",["title"=>$title])->with('result', $result);
  ...
  //updateProfile
	public function updateProfile(Request $request){
		...
		$orders_status = DB::table('admins')->where('id','=', Auth()->user()->id)->update([
  ...
  //updateProfile
	public function updateAdminPassword(Request $request){
		...
		$orders_status = DB::table('admins')->where('id','=', auth()->user()->myid)->update([
  ...
  //admins
	public function admins(Request $request){
		...
		$admins = DB::table('admins')
			->leftJoin('user_types','user_types.user_types_id','=','admins.role_id')
			->select('admins.*','user_types.*')
			->where('admins.role_id','>','10')
			->paginate(50);
		...
		return view("admin.admins.index",["title"=>$title])->with('result', $result);
	...
	public function addadmins(Request $request){
		...
		$title = Lang::get("labels.addadmin");
		...
		return view("admin.admins.add",["title"=>$title])->with('result', $result);
  ...
  //addnewadmin
	public function addnewadmin(Request $request){
		...
		$existEmail = DB::table('admins')->where('email', '=', $request->email)->get();
		if(count($existEmail)>0){
			...
			$customers_id = DB::table('admins')->insertGetId([
  ...
  //editadmin
	public function editadmin(Request $request){
		...
		$title = Lang::get("labels.EditAdmin");
		$myid        	 =   $request->id;
		...
		$admins = DB::table('admins')->where('id','=', $myid)->get();
		$zones = 0;
		...
		return view("admin.admins.edit",["title"=>$title])->with('result', $result);
	}
  ...
  //update admin
	public function updateadmin(Request $request){
		...
		//check email already exists
		$existEmail = DB::table('admins')->where([['email','=',$request->email],['id','!=',$myid]])->get();
		if(count($existEmail)>0){
			$errorMessage = Lang::get("labels.Email address already exist");
			return redirect()->back()->with('errorMessage', $errorMessage);
		}else{
			...
			$customers_id = DB::table('admins')->where('id', '=', $myid)->update($admin_data);
   ...
   public function profile(Request $request){
        ...
        $result['zones'] = $zones;
		$result['commonContent'] = $this->Setting->commonContent();
        return view("admin.admin.profile",["title"=>$title])->with('result', $result)->with('allimage', $allimage);
    }
     ...
     public function updatepassword(Request $request){
      ...
      //deleteProduct
    	public function deleteadmin(Request $request){
    		...
    		DB::table('admins')->where('id','=', $myid)->delete();
	...
	public function manageroles(Request $request){
		...
		$title = Lang::get("labels.manageroles");
		$language_id            				=   '1';
		...
		return view("admin.admins.roles.manageroles",["title"=>$title])->with('result', $result);
	...
	public function addadmintype(Request $request){
		$title = Lang::get("labels.addadmintype");
		...
		return view("admin.admins.roles.addadmintype",["title"=>$title])->with('result', $result);
	}
	...
	public function editadmintype(Request $request){
		$title = Lang::get("labels.EditAdminType");
		...
		$result['commonContent'] = $this->Setting->commonContent();
		return view("admin.admins.roles.editadmintype",["title"=>$title])->with('result', $result);
	}
	...
	public function addrole(Request $request){
		...
		if(count($roles)>0){
			...
			$vendor_view = $roles[0]->vendors_view;
			$vendor_create = $roles[0]->vendors_create;
			$vendor_update = $roles[0]->vendors_update;
			$vendor_delete = $roles[0]->vendors_delete;
                        
			$shop_view = $roles[0]->shops_view;
			$shop_create = $roles[0]->shops_create;
			$shop_delete = $roles[0]->shops_delete;
			$shop_update = $roles[0]->shops_update;
		...
		$result2[22]['link_name'] = 'Vendors';
		$result2[22]['permissions'] = array(
					'0'=>array('name'=>'vendors_view','value'=>$vendor_view),
					'1'=>array('name'=>'vendors_create','value'=>$vendor_create),
					'2'=>array('name'=>'vendors_update','value'=>$vendor_update),
					'3'=>array('name'=>'vendors_delete','value'=>$vendor_delete),
					);
			
                $result2[23]['link_name'] = 'Shops';
		$result2[23]['permissions'] = array(
					'0'=>array('name'=>'shops_view','value'=>$shop_view),
					'1'=>array('name'=>'shops_create','value'=>$shop_create),
					'2'=>array('name'=>'shops_update','value'=>$shop_update),
					'3'=>array('name'=>'shops_delete','value'=>$shop_delete),
					);
			

		$result['data'] = $result2;
		$result['commonContent'] = $this->Setting->commonContent();
		return view("admin.admins.roles.addrole",["title"=>$title])->with('result', $result);
	...
	public function addnewroles(Request $request){
		...
		$roles = DB::table('manage_role')->where('user_types_id',$request->user_types_id)->insert([
						...
						'vendors_view' => $request->vendors_view,
						'vendors_create' => $request->vendors_create,
						'vendors_delete' => $request->vendors_delete,
						'vendors_update' => $request->vendors_update,
                    
                                                'shops_view' => $request->shops_view,
                                                'shops_create' => $request->shops_create,
                                                'shops_delete' => $request->shops_delete,
                                                'shops_update' => $request->shops_update,
   ...
   //managerole
	public function categoriesroles(Request $request){
		$title = Lang::get("labels.CategoriesRoles");
		$result = array();
		$language_id = 1;

		$categories_role = DB::table('admins')->join('categories_role','categories_role.admin_id','=','admins.role_id')->where('admins.role_id','!=','1')->get();
		...
		$result['data'] = $data;
		$result['commonContent'] = $this->Setting->commonContent();
		return view("admin.admins.roles.category.index",["title"=>$title])->with('result', $result);
	}

  	//addCategoriesRoles
  	public function addCategoriesRoles(Request $request){

  			$title = Lang::get("labels.AddCategoriesRoles");
  			$result = array();
  			$language_id = 1;
  			...
  			$result['admins'] = DB::table('admins')->where('role_id','!=','1')->get();

  			$result['data'] = $categories_role;
			  $result['commonContent'] = $this->Setting->commonContent();
  			return view("admin.admins.roles.category.add",["title"=>$title])->with('result', $result);
  ...
  //editCategoriesRoles
	public function editCategoriesRoles(Request $request){
		...
		return view("admin.admins.roles.category.edit",["title"=>$title])->with('result', $result);
