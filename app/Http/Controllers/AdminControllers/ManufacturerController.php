...
class ManufacturerController extends Controller
{
    ...
    {
        $title = Lang::get("labels.Manufacturers");
        $manufacturers = $this->manufacturers->paginator(20);        
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.manufacturers.index",["title"=>$title])->with('manufacturers', $manufacturers)->with('result', $result);
    }

    public function add(Request $request)
    {
        $allimage = $this->images->getimages();
        $title = Lang::get("labels.AddManufacturer");
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.manufacturers.add", ["title"=>$title])->with('allimage', $allimage)->with('result', $result);
    }
    ...
    {
        $title = Lang::get("labels.EditManufacturers");
        $manufacturers_id = $request->id;
        $editManufacturer = $this->manufacturers->edit($manufacturers_id);
        $allimage = $this->images->getimages();        
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.manufacturers.edit", ["title"=>$title])->with('result', $result)->with('editManufacturer', $editManufacturer)->with('allimage', $allimage);
    }
    ...
    public function filter(Request $request)
    {
        ...
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.manufacturers.index", ["title"=>$title])->with('result', $result)->with('manufacturers', $manufacturers)->with('name', $name)->with('param', $param);
    }
