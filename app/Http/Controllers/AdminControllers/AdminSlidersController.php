...
class AdminSlidersController extends Controller
{
    ...
    {
        $title = Lang::get("labels.ListingSliders");
        ...
        return view("admin.settings.web.sliders.index", ["title"=>$title])->with('result', $result);
    }

    //addTaxClass
    public function addsliderimage(Request $request)
    {
        $title = Lang::get("labels.AddSliderImage");
        ...
        return view("admin.settings.web.sliders.add", ["title"=>$title])->with(['result' => $result, 'allimage' => $allimage]);
    }
    ...
    public function addNewSlide(Request $request)
    {
       ...
       try{
        if($request->hasFile('image_id')){
                $uploadImage = uploadImage($request->file('image_id'), 'web_slider','slider');
            }
    ...
    {
        $title = Lang::get("labels.EditSliderImage");
        $result = array();
        ...
        return view('admin.settings.web.sliders.edit', ["title"=>$title])->with(['result' => $result, 'allimage' => $allimage]);
    }
