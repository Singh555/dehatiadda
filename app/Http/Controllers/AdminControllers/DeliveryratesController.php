...
class DeliveryratesController extends Controller
{
    ...
    public function display(){
      $title = Lang::get("labels.Deliveryrates");
      $deliveryrates = $this->Deliveryrates->paginator();
      $result['commonContent'] = $this->Setting->commonContent();
      return view("admin.deliveryrates.index",["title"=>$title])->with('data', $deliveryrates)->with('result', $result);
    }

    public function add(Request $request){
      $title = Lang::get("labels.AddDeliveryrates");
      $result = array();
      $result['message'] = array();
      $result['languages'] = $this->myVarsetting->getLanguages();
      $result['zones'] = $this->Deliveryrates->Zones();
      $result['commonContent'] = $this->Setting->commonContent();
      return view("admin.deliveryrates.add",["title"=>$title])->with('result', $result);
    }
    ...
    public function edit(Request $request){
      $title = Lang::get("labels.EdiDeliveryrates");
      ...
      $result['zones'] = $this->Deliveryrates->Zones();
      $result['commonContent'] = $this->Setting->commonContent();
      return view("admin.deliveryrates.edit",["title"=>$title])->with('result', $result);
    }
    ...
    public function filter(Request $request){
        ...
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.deliveryrates.index",["title"=>$title])->with('result', $result)->with('data', $deliveryrates)->with('name', $name)->with('parameter', $param);
