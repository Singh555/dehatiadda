...
class ProductController extends Controller
{
    ...
    {
        $title = Lang::get("labels.reviews");
        $result = array();
        $data = $this->reviews->paginator();
        $result['reviews'] = $data;
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.reviews.index", ["title"=>$title])->with('result', $result);
    ...
    public function display(Request $request)
    {
        ...
        $title = Lang::get("labels.Products");
        ...
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.products.index", ["title"=>$title])->with('result', $result)->with('results', $results)->with('categories_id', $categories_id)->with('product', $product);

    }

    public function add(Request $request)
    {
        $title = Lang::get("labels.AddProduct");
        $language_id = '1';
        ...
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.products.add", ["title"=>$title])->with('result', $result)->with('allimage', $allimage);
    ...
    public function edit(Request $request)
    {
        ...
        $option .= '</ul>';
        $result['categories'] = $option;
        $title = Lang::get("labels.EditProduct");
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.products.edit", ["title"=>$title])->with('result', $result)->with('allimage', $allimage);
    ...
    //bulk inventory add form
     public function addinventoryBulk(Request $request)
    {
        $title = array('pageTitle' => Lang::get("labels.ProductInventory"));
        
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.products.inventory.bulk", $title)->with('result', $result);

    }
    
    //update bulk intentory
    public function updateInventoryBulk(Request $request) {
        $extensions = array("xls","xlsx","csv");

    $result = array($request->file('select_file')->getClientOriginalExtension());

    if(in_array($result[0],$extensions)){
        $this->products->updateInventoryBulk($request);
    }else{
        session()->flash('error', "file must be of type csv,xls,xlsx");
        return redirect()->back(); 
    }
    ...
    {
        $title = Lang::get("labels.ProductInventory");
        $result = $this->products->addinventoryfromsidebar();
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.products.inventory.add1", ["title"=>$title])->with('result', $result);
    ...
    public function displayProductImages(Request $request)
    {
        ...
        $title = Lang::get("labels.AddImages");
        $products_id = $request->id;
        $result = $this->products->displayProductImages($request);
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.products/images/index", ["title"=>$title])->with('result', $result)->with('products_id', $products_id);
    ...
    public function addProductImages($products_id)
    {
        ...
        $result['commonContent'] = $this->Setting->commonContent();
        return view('admin.products.images.add', ["title"=>$title])->with('result', $result)->with('products_id', $products_id)->with('allimage', $allimage);
    ...
    public function insertProductImages(Request $request)
    {
        ...
        return redirect('admin/products/images/display/'.$product_id)->with('product_id', $product_id);
    }
    ...
    public function updateproductimage(Request $request)
    {
        ...
        $title = Lang::get("labels.Manage Values");
        $product_id = $request->products_id;
        $result = $this->products->updateproductimage($request);
        return redirect('admin/products/images/display/'.$product_id);
    ...
    {
        $title = Lang::get("labels.AddAttributes");
        $result = $this->products->addproductattribute($request);
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.products.attribute.add", ["title"=>$title])->with('result', $result);
    }
    ...
    public function updateProductStatus(Request $request) {
        ...
        if($status == 1){
         ...
         if(!empty($id)){
                    ...
                    if (Products::updateProductStatus($id,$status)) {
                        DB::commit();
                        session()->flash('success', "Product has been $statusTitle successfully!");
                    } else{
                        Log::error("Error Occured at Product status $statusTitle update ");
