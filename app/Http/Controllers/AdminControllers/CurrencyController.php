...
class CurrencyController extends Controller
{
  ...
  public function display(){
    $title = Lang::get("labels.Currency");
    $currencies = $this->currencies->paginator();
    $result['commonContent'] = $this->Setting->commonContent();
    return view("admin.currencies.index",["title"=>$title])->with('result', $result)->with('currencies', $currencies);
  }
  ...
  public function filter(Request $request){
    ...
    return view("admin.categories.index", ["title"=>$title])->with(['categories'=> $categories, 'name'=> $request->FilterBy, 'param'=> $request->parameter]);
  }

  public function add(Request $request){
    $title = Lang::get("labels.Add Currency");
    $currencies = DB::table('currency_record')->get();
    $result['commonContent'] = $this->Setting->commonContent();
    return view("admin.currencies.add",["title"=>$title])->with(['currencies' => $currencies, 'result' => $result]);
  }
  ...
  public function edit($currency_id){
    $title = Lang::get("labels.Edit Currency");
    $result = array();
    $currencies = DB::table('currency_record')->get();
    ...
    $result['warning'] = 0;
    $result['commonContent'] = $this->Setting->commonContent();
    return view("admin.currencies.edit",["title"=>$title])->with('result', $result)->with('currencies',$currencies);
   }
