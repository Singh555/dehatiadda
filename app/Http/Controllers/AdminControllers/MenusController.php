...
class MenusController extends Controller
{
    ...
    {
        $title = Lang::get("labels.menus");
        $result['menus'] = Menus::menus();  
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.menus.index", ["title"=>$title])->with('result', $result);
    }

    public function addmenus(Request $request)
    {
        $language_id =  1;
        $title = Lang::get("labels.addmenus");
        ...
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.menus.add", ["title"=>$title])->with('result', $result);
    }
    ...
    public function editmenu(Request $request, $id)
    {
        ...
        $result['products'] = $this->Products->getter($language_id);
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.menus.edit", ["title"=>$title])->with('result', $result);
    }
