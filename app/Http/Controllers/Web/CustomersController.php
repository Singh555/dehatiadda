...
class CustomersController extends Controller
{
    ...
    public function __construct(
     ...
     // Otp Sending and registration if not already
        public function usualLoginSendOtpAjax(Request $request){
            $data = array();
         $rsult = Customer::usualLoginSendOtp($request);
         if($rsult){
             session()->put("success","Otp has been Sent on your Number");
                $data['code'] = 200;
                $data['status'] = 'success';
                $data['message'] = 'Otp has been Sent on your Number';
         }else{
              Log::debug("Opt Send Failed");
            $data['code'] = 201;
            $data['status'] = 'error';
            $data['message'] = 'Opt Sending Failed';
         }
        ...
        public function validateReferralCodeAjax(Request $request){
        ...
        public function verifyLogin(Request $request){
         $result = Customer::verifyLogin($request);
         if($result){
        ...
        if (auth()->guard('customer')->check()) {
            ...
            if(Customer::primeSignup($request)){
            $title = array('pageTitle' => "Verify Prime Signup");
            $result = array();
            $result['commonContent'] = $this->index->commonContent();
            return view("auth.verifyPrimeSignup", ['title' => $title, 'final_theme' => $final_theme])->with('result', $result)
                 ->with('fromData', $request->except(['_token']));
            }
            return redirect('login');
        }
        ...
        public function primeSignupVerify(Request $request){
         $result = Customer::primeSignupVerify($request);
        if($result){
         if (!empty(session('previous'))) {
                return Redirect::to(session('previous'));
            } else {
        ...
        if (auth()->guard('customer')->check()) {
            ...
            if(Customer::normalSignup($request)){
            $title = array('pageTitle' => "Verify Normal Signup");
            $result = array();
            $result['commonContent'] = $this->index->commonContent();
            return view("auth.verifyNormalSignup", ['title' => $title, 'final_theme' => $final_theme])->with('result', $result)
                 ->with('fromData', $request->except(['_token']));
            }
            return redirect('login');
        }
        ...
        public function normalSignupVerify(Request $request){
            $result = Customer::normalSignupVerify($request);
        if($result){
         if (!empty(session('previous'))) {
                return Redirect::to(session('previous'));
            } else {
